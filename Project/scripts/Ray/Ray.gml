...
function Ray(origin, direction) constructor{
	...
	static dbug_draw = function(col=c_white){
		...
		//vertex_add(vbuff, s.x,	s.y,		s.z,			c_yellow);
		//vertex_add(vbuff, origin.x,	origin.y,		origin.z,			c_lime);
		
		//vertex_add(vbuff, p.x,	p.y,		p.z,			c_orange);
		//vertex_add(vbuff, p.x,	p.y,		p.z-32,			c_fuchsia);
		//if (i<player.collider.shape.radius and t>0){
		//vertex_add(vbuff, i1.x,	i1.y,		i1.z,			c_red);
		//vertex_add(vbuff, i1.x,	i1.y,		i1.z-32,			c_red);
		//vertex_add(vbuff, i2.x,	i2.y,		i2.z,			c_blue);
		//vertex_add(vbuff, i2.x,	i2.y,		i2.z-32,			c_blue);
		//}
		
		//vertex_add(vbuff, origin.x+e.x,	origin.y+e.y,		origin.z+e.z,			c_lime);
		//vertex_add(vbuff, origin.x,	origin.y,		origin.z,			c_yellow);
	//////////////////////////////////////////////
